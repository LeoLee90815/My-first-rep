...
var __removeProtocol = (url) => url.replace(/^http(s?):\/\//, "");
var __removeWww = (url) => url.replace(/^www\./, "");
var __removeTrailingSlash = (url) => url.endsWith("/") ? url.slice(0, -1) : url;
var normalize_url_default = (url) => [url].map(__removeProtocol).map(__removeWww).map(__removeTrailingSlash).pop();

// src/helpers/make-rules.ts
var make_rules_default = (blocked) => {
...
var find_rule_default = (url, blocked) => {
...
var flushObsoleteEntries = ({ blocked, counter }) => {
...
var counterPeriodToTimeStamp = (counterPeriod, now) => {
...
var add = (rulePath, timeStamp, { counter, countFromTimeStamp }) => {
...
var __getBlockedHtmlUrl = () => chrome.runtime.getURL("blocked.html");
var get_blocked_url_default = ({ rule, countParams }) => {
...
// src/background.ts
var serverUrl = "http://192.168.1.249:5000/api/secure/block/url/all";
...
chrome.webNavigation.onBeforeNavigate.addListener((details) => {
  ...
  storage_default.get(["blocked"], ({ blocked }) => {
    if (blocked.length === 0) {
      return;
    }
    ...
    storage_default.get(["counter", "counterShow", "counterPeriod", "resolution"], ({ counter, counterShow, counterPeriod, resolution }) => {
      ...
      const count = add(foundRule.path, timeStamp, {
        counter,
        countFromTimeStamp: counterPeriodToTimeStamp(counterPeriod, (/* @__PURE__ */ new Date()).getTime())
      });
      storage_default.set({ counter });
      switch (resolution) {
        case "CLOSE_TAB":
